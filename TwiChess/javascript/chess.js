...
const ls_doCastle =  localStorage.getItem("doCastle");// true / false
...
let gamerules = {
    doCastle: ((ls_doCastle == "true" || ls_doCastle == null)? [true, true]: [false, false]),
...
let wCastle = gamerules.doCastle;
let bCastle = gamerules.doCastle;
...
function removeLocalStorage(){
    ...
    localStorage.removeItem("doCastle");
...
function timerChange(mode = "update") {
    if (mode == "update"){
        if (whiteTurn) {
            wTimer--;
            wTimerHTML.innerHTML = `${Math.floor(wTimer / 60)}:${(wTimer % 60).toString().padStart(2, "0")}`;
            if (wTimer == 0) {
                gameOver("win", "Black Wins due to timeout!");
                stopTimer();
                return;
            }
        } 
            ...
            bTimer--;
            bTimerHTML.innerHTML = `${Math.floor(bTimer / 60)}:${(bTimer % 60).toString().padStart(2, "0")}`;
            if (bTimer == 0) {
                gameOver("win", "White Wins due to timeout!"); 
                stopTimer(); 
                return;
            }
        }
    clearTimeout(timeTick);
    timeTick = setTimeout(timerChange, 1000);
    } 
